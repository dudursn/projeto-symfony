...
class UsuariosEventListener implements EventSubscriberInterface
{
    /**
    ...
    public function __construct(UsuariosMapper $usuariosMapper, RequestStack $requestStack) {
    ...
    public function onPostSetData(FormEvent $event)
    {
        ...
        if($this->request->isMethod('POST') && !$this->request->request->get('pesquisa_reset')) {
            if(is_null($this->postKey)) {
                ...
                $usuarioId = is_array($post) && isset($post[$form_name]) && $post[$form_name]?$post[$form_name]:null;
            } else if($this->postKey == '') {
                $usuarioId = $this->request->request->get($form_name, null);
            } else {
                $post = $this->request->request->get($this->postKey);
                $usuarioId = is_array($post) && isset($post[$form_name]) && $post[$form_name]?$post[$form_name]:null;
            }
            
            $postUsuario = $this->usuariosMapper->find($usuarioId);
            if($postUsuario)
                $choices[] = $postUsuario;
        }
        
        if(($data instanceof \App\Entity\Usuarios) && $data->getId())
            $choices[] = $data;
